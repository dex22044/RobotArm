...
void ArmIK::AddJoint(float len) {
	Joint newJoint = { 0 };
	newJoint.len = len;
	newJoint.limited = false;
	joints.push_back(newJoint);
}

void ArmIK::AddJoint(float len, float minAngle, float maxAngle) {
	Joint newJoint = { 0 };
	newJoint.len = len;
	newJoint.limited = true;
	newJoint.minAngle = minAngle;
	newJoint.maxAngle = maxAngle;
	joints.push_back(newJoint);
}
...
void ArmIK::ForwardKinematics(int startJoint) {
	...
	for (int i = startJoint; i < joints.size(); i++) {
		if (!joints[i].limited) continue;
		if (joints[i].localAngle < joints[i].minAngle) joints[i].localAngle = joints[i].minAngle;
		if (joints[i].localAngle > joints[i].maxAngle) joints[i].localAngle = joints[i].maxAngle;
	}
